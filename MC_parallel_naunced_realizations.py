...
# -*- coding: utf-8 -*-
"""
Created on Fri May  2 12:15:59 2025
...
def save_checkpoint(realizations, scores, index):
    """
    Saves the current realizations, scores, and the index to checkpoint files.
    """
    with open(checkpoint_file_realizations, 'wb') as f:
        pickle.dump(realizations, f)

    with open(checkpoint_file_scores, 'wb') as f:
        pickle.dump(scores, f)

    with open(checkpoint_file_index, 'wb') as f:
        pickle.dump(index, f)
...
def load_checkpoint():
    ...
    if os.path.exists(checkpoint_file_realizations) and \
       ...
       os.path.exists(checkpoint_file_index):
        ...
        return realizations, scores, index
    else:
        return None, None, 0  # No checkpoint found, start from the beginning

# Load checkpoint (if any)
accepted_realizations, accepted_scores, start_index = load_checkpoint()

# Initialize lists if no checkpoint
if accepted_realizations is None:
...
#You should not transpose the matrix! python splicing is different than matlab splicing!
...
rho_fib1 = np.random.uniform(1.556,3.472,MC)*1e-22
...
rho_fib2 = np.random.uniform(1.556,3.472,MC)*1e-22
...
rho_fib3 = np.random.uniform(1.556,3.472,MC)*1e-22
...
rec_number_fib1 = np.random.uniform(8350,14850,MC)/rec_const
...
rec_number_fib2 = np.random.uniform(8350,14850,MC)/rec_const
...
rec_number_fib3 = np.random.uniform(8350,14850,MC)/rec_const
...
if __name__ == "__main__":
    ...
    start_index = 0  # Define or load from checkpoint


    evaluate_partial = partial(
        evaluate_realization,
        R=realizations,
        I_emp=interaction_data,
        neighbours=neighbours_info.iloc[:, 1].tolist(),
        mesh_prop=mesh_properties,
        cell_dens=cell_densities,
    )
    ...
    with Pool(processes=num_workers) as pool:
        for i, result in enumerate(pool.imap(evaluate_partial, range(start_index, MC))):
            if result:
                r_i, score = result
                accepted_realizations.append(r_i)
                accepted_scores.append(score)

            if (i + start_index) % 5e3 == 0:
                print(f"Checkpointing after realization {i + start_index}")
                save_checkpoint(accepted_realizations, accepted_scores, i + start_index)

    if accepted_realizations:
        np.save("accepted_realizations.npy", np.array(accepted_realizations))
        np.save("accepted_scores.npy", np.array(accepted_scores))
    else:
        print("No accepted realizations.")


with open("checkpoint_realizations.pkl", "rb") as f:
   updating_realizations = pickle.load(f)
with open("checkpoint_index.pkl", "rb") as f:
      updating_index = pickle.load(f)
with open("checkpoint_scores.pkl", "rb") as f:
      updating_scores = pickle.load(f)
saved_accepted_realizations = np.load('accepted_realizations.npy')
